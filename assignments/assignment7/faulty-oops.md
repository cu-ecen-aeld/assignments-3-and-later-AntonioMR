...
Data abort info:
  ...
  CM = 0, WnR = 1
user pgtable: 4k pages, 39-bit VAs, pgdp=000000004266e000
...
sp : ffffffc008c8bd80
x29: ffffffc008c8bd80 x28: ffffff80026b8cc0 x27: 0000000000000000
x26: 0000000000000000 x25: 0000000000000000 x24: 0000000000000000
x23: 0000000040000000 x22: 000000000000000c x21: 00000055839a2270
x20: 00000055839a2270 x19: ffffff8002637700 x18: 0000000000000000
x17: 0000000000000000 x16: 0000000000000000 x15: 0000000000000000
x14: 0000000000000000 x13: 0000000000000000 x12: 0000000000000000
x11: 0000000000000000 x10: 0000000000000000 x9 : 0000000000000000
x8 : 0000000000000000 x7 : 0000000000000000 x6 : 0000000000000000
x5 : 0000000000000001 x4 : ffffffc0006f7000 x3 : ffffffc008c8bdf0
...
---[ end trace a44d4a841f458cbb ]---
```

## Debugging tips

In first point the msg warn us that we are dealing with a null pointer error
```shell
Unable to handle kernel NULL pointer dereference at virtual address 0000000000000000
```

We can observe that the `pc` register points a memory area near 'faulty_write' function.
```shell
pc : faulty_write+0x14/0x20 [faulty]
```

We must check this function to find the null pointer thas is being addressing.

Also we can obtain the assembler code and observe the code where the error occurs if we use objdump

If objdump is installed in the system running in qemu, run:

```shell
objdump -S /lib/modules/5.15.18/extra/faulty.ko
```

If not, we can use the cross-platform version generated by buildroot. Assuming that we are we are in the buildroot directory, run:

```shell
./output/host/aarch64-buildroot-linux-uclibc/bin/objdump -S ./output/target/lib/modules/5.15.18/extra/faulty.ko 
```

this get us the next assembly code:
...
Again, if we look at the error given 

```shell
pc : faulty_write+0x14/0x20 [faulty]
```

We can found the instruction that is causing the error (address 0x14)

```asm
  14:	b900003f 	str	wzr, [x1]
```

We are trying to store the value of register wzr in the address pointed by register x1, which has been previously loaded with 0 (addr 4)

```asm
   4:	d2800001 	mov	x1, #0x0                   	// #0
```

So the null pointer error given when trying to write to the /dev/faulty device